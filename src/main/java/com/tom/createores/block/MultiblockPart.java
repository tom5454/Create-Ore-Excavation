...
public interface MultiblockPart extends IWrenchable {
	...
	ItemStack pickBlock(BlockState state, HitResult target, BlockGetter level, BlockPos pos, Player player);
	...
	public static interface MultiblockGhostPart extends MultiblockPart {
		...
		@Override
		public default ItemStack pickBlock(BlockState state, HitResult target, BlockGetter level, BlockPos pos, Player player) {
			for (int i = 0;i<5 && state.getBlock() instanceof MultiblockGhostPart;i++) {
				Direction d = ((MultiblockGhostPart)state.getBlock()).getParentDir(state);
				pos = pos.relative(d, 1);
				state = level.getBlockState(pos);
			...
			if (state.getBlock() != this && state.getBlock() instanceof MultiblockPart) {
				...
				else return d.pickBlock(state, target, level, pos, player);
			}
	...
	public static interface MultiblockMainPart extends MultiblockPart {
		...
		public static enum MultiblockPartType {
			...
			ITEM_IN   (IOBlock.Type.ITEM_IN   ),
			ITEM_OUT  (IOBlock.Type.ITEM_OUT  ),
			FLUID_IN  (IOBlock.Type.FLUID_IN  ),
			FLUID_OUT (IOBlock.Type.FLUID_OUT ),
			ENERGY_IN (IOBlock.Type.ENERGY_IN ),
			ENERGY_OUT(IOBlock.Type.ENERGY_OUT),
			;
			...
			private MultiblockPartType(IOBlock.Type type) {
